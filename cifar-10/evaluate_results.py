...
# root_path = 'cifar-10/eval-t-100'
root_path = 'cifar-10/eval-n100'
m_prefix = ''
save_heatmaps = True
title = True
x = 10
...
def save_stats(stats_path,
  ...
  if con_classes is not None:
    ...
    for i in range(num_classes):
        ...
        axes[i].set_ylim(0, 10)  # setting a fixed y-axis limit for better comparison
            ...
            for subfolder in subfolders:
                ...
                if os.path.exists(m_path):
                    ...
                    for m_png in m_pngs[:1]:
                        ...
                        img_array = convert_to_grayscale(np.array(img))

                        m_img_path = os.path.join(m_path, m_png)
                        m_img = Image.open(m_img_path)
                        m_img_array = convert_to_grayscale(np.array(m_img))
                        ...
                        with open(m_img_json_path, 'r') as f:
                          ...
                          stylemix_seed = data['iteration-count']
                        ...
                        if save_heatmaps:
                          ...
                          if overwite_heatmaps or not os.path.exists(heatmap_path):
                            ...
                            if img_l2 < m_img_l2:
                              ...
                              color = 'black'
                              m_color = 'red'
                            if subfolder.endswith('-bf'):
                              class_name = f_mnist[int(subfolder[:-3])] + '-bf'
                            else:
                              class_name = f_mnist[int(subfolder)]
                            axs[0].set_title(f'Original Image - Class {f_mnist[int(class_folder)]} - L2: {img_l2}', color=color)

                            # Display modified image
                            axs[1].imshow(m_img, cmap='gray')
                            axs[1].set_title(f'Modified Image - Class {class_name} - L2: {m_img_l2}', color=m_color)
