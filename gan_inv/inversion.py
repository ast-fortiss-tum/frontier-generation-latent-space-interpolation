import math
import os
from stylegan import renderer as renderer
import torch
...
import imageio
from mnist.config import INIT_PKL
...
def inverse_image(
    ...
    with torch.no_grad():
        ...
        if class_idx is not None:
            label = label[:, class_idx] = 1

        w_samples = g_ema.mapping(noise_sample,label, truncation_psi=trunc_psi, truncation_cutoff=trunc_cutoff)
...
class PTI:
    ...
    def train(self,img,w_plus=False):
        if torch.is_tensor(img) == False:
            ...
            real_img = transform(img).to('cuda').unsqueeze(0)
        # print('#########################')
        # print(self.g_ema.mapping)
        # print(img.size)
        # print(self.percept)
        # print(self.g_ema.img_resolution)
        # print(w_plus)
        # print('#########################')
